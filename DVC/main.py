...
def train(epoch, global_step):
    ...
    for batch_idx, input in enumerate(train_loader):
        ...
        if (batch_idx % print_step)== 0 and bat_cnt > 1:
            wandb.log({
                'lr': cur_lr,
                'rd_loss': sumloss / cal_cnt,
                'psnr': sumpsnr / cal_cnt,
                'warppsnr': sumwarppsnr / cal_cnt,
                'interpsnr': suminterpsnr / cal_cnt,
                'bpp': sumbpp / cal_cnt,
                'bpp_feature': sumbpp_feature / cal_cnt,
                'bpp_z': sumbpp_z / cal_cnt,
                'bpp_mv': sumbpp_mv / cal_cnt
            }, step=global_step)
...
if __name__ == "__main__":
    ...
    train_dataset = DataSet('H:/Data/vimeo_septuplet/vimeo_septuplet/mini_dvc_test_10k.txt')
