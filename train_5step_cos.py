...
train_args = {
    ...
    'describe': "按照TCM5步配置训练，增加学习率调整策略(warmup+余弦)，增加fine-tuning",
...
class Trainer(Module):
    ...
    def schedule(self):
        ...
        if self.current_epoch < train_args["warmup_border"]:
            ...
            self.scheduler = optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=decay_interval, eta_min=1e-8, verbose=True)
            pass
...
if __name__ == "__main__": 
    ...
    for epoch in range(train_args['epochs']):
        ...
        # 余弦退火
        if epoch >= train_args["decay_border"]:
