...
class DCVC_net(nn.Module):
    ...
    def forward(self, referframe, input_image):
        ...
        # [1]
        estmv = self.opticFlow(input_image, referframe)
        mvfeature = self.mvEncoder(estmv) # 1.1. 由于需要运动补偿，estmv 需要压缩送到解码端（需要过自编码器）；
        # 1.2. mvEncoder 也是一个特征提取器，输出可以进一步获取 hyperprior 来完成 estmv 的熵估计
        z_mv = self.mvpriorEncoder(mvfeature) # z 在此处并非指 latent，而是指 hyperprior，对应 1.2
        compressed_z_mv = torch.round(z_mv) # 量化（离散化）之后才能熵编码
        
        # [2]
        params_mv = self.mvpriorDecoder(compressed_z_mv) # param_mv 是用来估计mv的lantent的分布的，即hyperprior，在解码端也需要用，所以需要量化（压缩）
        
        # [3]
        quant_mv = torch.round(mvfeature) # 对应 1.1
        ctx_params_mv = self.auto_regressive_mv(quant_mv) # 为什么要自回归？—— 帧内的空间上下文信息
        gaussian_params_mv = self.entropy_parameters_mv( # 拼接 mvPriorDecoder([mvPriorEncoder(mvfeature)]) 和 auto_regressive([mvfeature]) 
            torch.cat((params_mv, ctx_params_mv), dim=1) # hyperprior 和 自回归的结果
        )
        means_hat_mv, scales_hat_mv = gaussian_params_mv.chunk(2, 1) # 从预测的参数中分离出均值和标准差（位置和规模）

        # [4]
        quant_mv_upsample = self.mvDecoder_part1(quant_mv) # 某种意义上是上采样，更应该理解为从 latent 恢复 mv
        quant_mv_upsample_refine = self.mv_refine(referframe, quant_mv_upsample) # refine 的设计？
        ...
        # 熵模型部分 （Figure 3）
        temporal_prior_params = self.temporalPriorEncoder(context) # frame latent 的 hyperprior（用参考帧+mv）
        feature = self.contextualEncoder(torch.cat((input_image, context), dim=1)) # 利用 context 改善压缩效果的操作就是 把 context 和 input_image 一起送进自编码器
        z = self.priorEncoder(feature) # frame latent 的 hyperprior（用当前帧+参考帧+mv）
        compressed_z = torch.round(z) 
        ...
        recon_image = self.contextualDecoder_part2(torch.cat((recon_image_feature, context), dim=1))
        # 如果用 compressed_y_renorm 和 context 就可以重建图像，那为什么要估计 means_hat 和 scales_hat？—— 量化之后是要熵编码，熵编码需要分布
