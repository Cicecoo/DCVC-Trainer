...
from .DVC_Spynet import ME_Spynet as DVC_Spynet # modified

# def save_model(model, iter):
#     torch.save(model.state_dict(), "./snapshot_241231/iter{}.model".format(iter))
...
class DCVC_net(nn.Module):
    def __init__(self):
        ...
        self.contextualEncoder = contextualEncoder_net(out_channel_N, out_channel_M)
        self.contextualDecoder_part1 = contextualDecoder_part1_net(out_channel_M, out_channel_N)
        self.contextualDecoder_part2 = contextualDecoder_part2_net(out_channel_N)
...
class ResBlock_LeakyReLU_0_Point_1(nn.Module):
    def __init__(self, d_model):
        ...
        self.l1 = nn.Conv2d(d_model, d_model, 3, stride=1, padding=1)
        torch.nn.init.xavier_normal_(self.l1.weight.data, (math.sqrt(2 * d_model / (d_model + d_model))))
        torch.nn.init.constant_(self.l1.bias.data, 0.01)
        self.r1 = nn.LeakyReLU(0.1, inplace=True)
        self.l2 = nn.Conv2d(d_model, d_model, 3, stride=1, padding=1)
        torch.nn.init.xavier_normal_(self.l2.weight.data, (math.sqrt(2 * d_model / (d_model + d_model))))
        torch.nn.init.constant_(self.l2.bias.data, 0.01)
        self.r2 = nn.LeakyReLU(0.1, inplace=True)
...
class contextualEncoder_net(nn.Module):
    def __init__(self, out_channel_N, out_channel_M):
        super(contextualEncoder_net, self).__init__()
        self.conv1 = nn.Conv2d(out_channel_N+3, out_channel_N, 5, stride=2, padding=2)
        torch.nn.init.xavier_normal_(self.conv1.weight.data, (math.sqrt(2 * (out_channel_N + out_channel_N + 3) / (out_channel_N + out_channel_N))))
        torch.nn.init.constant_(self.conv1.bias.data, 0.01)
        self.gdn1 = GDN(out_channel_N)
        self.resblock1 = ResBlock_LeakyReLU_0_Point_1(out_channel_N)
        self.conv2 = nn.Conv2d(out_channel_N, out_channel_N, 5, stride=2, padding=2)
        torch.nn.init.xavier_normal_(self.conv2.weight.data, (math.sqrt(2)))
        torch.nn.init.constant_(self.conv2.bias.data, 0.01)
        self.gdn2 = GDN(out_channel_N)
        self.resblock2 = ResBlock_LeakyReLU_0_Point_1(out_channel_N)
...
class subpel_conv3x3(nn.Module):
    def __init__(self, in_channel, out_channel, r):
        super(subpel_conv3x3, self).__init__()
        self.conv = nn.Conv2d(in_channel, out_channel * r ** 2, kernel_size=3, padding=1)
        torch.nn.init.xavier_normal_(self.conv.weight.data, (math.sqrt(2)))
        torch.nn.init.constant_(self.conv.bias.data, 0.01)
        self.pixelshuffle = nn.PixelShuffle(r)

    def forward(self, x):
        return self.pixelshuffle(self.conv(x))

class contextualDecoder_part1_net(nn.Module):
    def __init__(self, out_channel_M, out_channel_N):
        super(contextualDecoder_part1_net, self).__init__()
        self.subpel_conv1 = subpel_conv3x3(out_channel_M, out_channel_N, 2)
        self.igdn1 = GDN(out_channel_N, inverse=True)
        self.subpel_conv2 = subpel_conv3x3(out_channel_N, out_channel_N, 2)
        self.igdn2 = GDN(out_channel_N, inverse=True)
        self.resblock1 = ResBlock_LeakyReLU_0_Point_1(out_channel_N)
        self.subpel_conv3 = subpel_conv3x3(out_channel_N, out_channel_N, 2)
        self.igdn3 = GDN(out_channel_N, inverse=True)
        self.resblock2 = ResBlock_LeakyReLU_0_Point_1(out_channel_N)
        self.subpel_conv4 = subpel_conv3x3(out_channel_N, out_channel_N, 2)

    def forward(self, x):
        x = self.igdn1(self.subpel_conv1(x))
        x = self.igdn2(self.subpel_conv2(x))
        x = self.resblock1(x)
        x = self.igdn3(self.subpel_conv3(x))
        x = self.resblock2(x)
        return self.subpel_conv4(x)

class contextualDecoder_part2_net(nn.Module):
    def __init__(self, out_channel_N):
        super(contextualDecoder_part2_net, self).__init__()
        self.conv1 = nn.Conv2d(out_channel_N*2, out_channel_N, 3, stride=1, padding=1)
        torch.nn.init.xavier_uniform_(self.conv1.weight.data)
        torch.nn.init.constant_(self.conv1.bias.data, 0.0)
        self.resblock1 = ResBlock(out_channel_N, out_channel_N, 3)
        self.resblock2 = ResBlock(out_channel_N, out_channel_N, 3)
        self.conv2 = nn.Conv2d(out_channel_N, 3, 3, stride=1, padding=1)
